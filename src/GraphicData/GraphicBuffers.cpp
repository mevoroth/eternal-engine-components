...
void GraphicBuffers::Initialize(_In_ Device& DeviceObj)
{
	...
	_GraphicBuffersDescriptorHeap = CreateDescriptorHeap(DeviceObj, ROOT_SIGNATURE_PARAMETER_RENDER_TARGET, GRAPHIC_BUFFER_COUNT, (RootSignatureAccess)(ROOT_SIGNATURE_IA | ROOT_SIGNATURE_VS | ROOT_SIGNATURE_PS));
	
	for (uint32_t GraphicBufferIndex = 0; GraphicBufferIndex < GRAPHIC_BUFFER_NO_DEPTH_COUNT; ++GraphicBufferIndex)
	{
		_GraphicBuffersHeap[GraphicBufferIndex]		= CreateHeap(DeviceObj, VULKAN_FORMAT_SIZES[GraphicBufferFormats[GraphicBufferIndex]] * Size, 1, true, false, false, false);
	...
	_GraphicBuffersHeap[GRAPHIC_BUFFER_DEPTH]		= CreateHeap(DeviceObj, VULKAN_FORMAT_SIZES[GraphicBufferFormats[GRAPHIC_BUFFER_DEPTH]] * Size, 1, true, false, false, false);
