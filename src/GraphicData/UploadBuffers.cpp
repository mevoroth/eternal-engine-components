...
void UploadBuffers::Initialize(_In_ Device& DeviceObj)
{
	...
	for (uint32_t UploadBufferIndex = 0; UploadBufferIndex < UPLOAD_BUFFER_PER_HEAP; ++UploadBufferIndex)
	{
		...
		Resource* UploadBuffer		= CreateResource(DeviceObj, *_UploadHeaps[UPLOAD_BUFFER_TEXTURE], UploadBufferName, sizeof(uint32_t), 4096 * 4096 * sizeof(uint32_t), BUFFER_COPY_READ);
